...
protected:
    ...
    UPROPERTY(EditAnywhere, float, ShadowResolutionScale, = 0.0f)

    // 그림자가 객체 표면에 겹쳐 보이지 않도록 살짝 밀어내는 오프셋 값
    UPROPERTY(EditAnywhere, float, ShadowBias, = 0.0f)

    // 표면 기울기에 따라 동적으로 적용되는 추가 바이어스 (기울어진 면에서의 셀프섀도잉 방지)
    UPROPERTY(EditAnywhere, float, ShadowSlopeBias, = 0.0f)

    // 그림자 경계의 선명도를 조절하는 값 (값이 클수록 더 날카로운 그림자 경계)
    UPROPERTY(EditAnywhere, float, ShadowSharpen, = 0.0f)
public:
